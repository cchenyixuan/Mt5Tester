...
class Chart:
    def __init__(self, coin_pair="EURUSD", time_interval="15m"):
        ...
        while self.dm.candles[coin_pair][self._position_of_0][0]:
            self._position_of_0 += 1
        self._max = np.max(self.dm.candles[coin_pair][:min(self._position_of_0, 100000)])
        self._min = np.min(self.dm.candles[coin_pair][:min(self._position_of_0, 100000)])
        self.draw_candles(900*(self.dm.candles[coin_pair]-self._min)/(self._max-self._min)-450)
    ...
    def draw_candles(self, data: np.ndarray) -> None:
        ...
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, sbo)
